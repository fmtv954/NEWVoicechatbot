...
export async function POST(request: NextRequest) {
  try {
    ...
    const { agent_id, campaign_id, first_name, last_name, email, phone, reason, transcript, call_id } = body

    if (!agent_id || !campaign_id || !first_name || !last_name || !email || !phone || !reason || !transcript) {
