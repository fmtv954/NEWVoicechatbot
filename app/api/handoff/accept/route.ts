...
export async function POST(request: NextRequest) {
  try {
    ...
    let leadStatus: 'not_applicable' | 'found' | 'missing' | 'error' = 'not_applicable'
    let leadStatusReason: string | undefined

    if (ticket.call_id) {
      leadStatus = 'missing'
      leadStatusReason = 'lead_not_found_for_call'
      ...
      if (leadEventError) {
        ...
        leadStatus = 'error'
        leadStatusReason = 'lead_event_lookup_failed'
      }
      ...
      if (leadId) {
        ...
        if (leadError) {
          ...
          leadStatus = 'error'
          leadStatusReason = 'lead_lookup_failed'
        } else if (lead) {
          leadInfo = lead
          leadStatus = 'found'
          leadStatusReason = undefined
        }
      } else if (leadEvent) {
        leadStatusReason = 'lead_event_missing_lead_id'
      } else if (!leadEventError) {
        leadStatusReason = 'lead_event_not_found'
      }
    }
    ...
    return NextResponse.json({
      ...
      lead_status_reason: leadStatusReason,
