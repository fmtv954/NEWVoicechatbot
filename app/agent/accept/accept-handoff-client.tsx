...
export default function AcceptHandoffClient() {
  ...
  const [status, setStatus] = useState<
    'loading' | 'accepted' | 'error' | 'expired' | 'already_accepted'
  >('loading')
  ...
  function toggleMute() {
    if (roomRef.current) {
      const audioTrack = roomRef.current.localParticipant.audioTrackPublications.values().next()
        .value?.track
      if (audioTrack) {
        if (isMuted) {
          audioTrack.unmute()
  ...
  const hasLeadDetails = Boolean(
    leadDetails &&
      [leadDetails.first_name, leadDetails.last_name, leadDetails.email, leadDetails.phone].some(
        Boolean
      )
  )
                ...
                <div className="flex flex-col items-center justify-center py-12 gap-4">
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-xs text-muted-foreground mt-4">
                      This handoff link has expired. Please ask the customer to request a new
                      handoff.
                    </p>
                  </div>
                </div>
                  ...
                  <div className="mb-6 rounded-lg border border-blue-200 bg-blue-50 p-4">
                    ...
                    {roomData.lead_status && roomData.lead_status !== 'found' && (
                      <div className="mb-4 rounded-md border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900">
                        <Badge
                          variant="outline"
                          className="mb-2 border-amber-300 bg-amber-100 text-amber-900"
                        >
                    ...
                    {hasLeadDetails ? (
                      ...
                      <div className="flex items-start gap-2 rounded-md border border-blue-200 bg-white/70 p-3 text-sm text-blue-900">
                        <AlertCircle className="mt-0.5 h-4 w-4 flex-shrink-0" />
                        <p>
                          We don't have saved lead details for this caller. Please confirm their
                          name and contact information manually before proceeding.
                        </p>
                      </div>
                    )}
                  </div>
                      ...
                      <>
                        ...
                        <div className="text-center">
                          ...
                          <p className="text-sm text-muted-foreground mt-2">
                            You are now talking with the caller
                          </p>
                        </div>
            ...
            <div className="space-y-2 max-h-[400px] overflow-y-auto custom-scrollbar">
              {events.length === 0 ? (
                ...
                events.map((event, index) => (
                  <div
                    key={index}
                    className="bg-slate-800 rounded-lg p-3 text-xs border border-slate-700"
                  >
                    <div className="flex items-start justify-between gap-2 mb-1">
                      <span className={`font-mono font-semibold ${getEventColor(event.type)}`}>
                        {event.type}
                      </span>
